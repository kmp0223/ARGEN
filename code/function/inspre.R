...
inspre_worker <- function(X, W = NULL, rho = 1.0, lambda = 0.01,
                          ...
                          constraint = "UV", DAG = FALSE, beta_obs = NULL) {
  D <- nrow(X)
  if (is.null(W)) {
  ...
  for (iter in 1:its) {
    if (constraint == "UV") {
      U_next <- fit_U_UV_const(WX, W, V, U, theta, rho, 0, solve_its, DAG, ncores, beta_obs)
      V_next <- fit_V_UV_const(V, U_next, theta, rho, lambda, solve_its, DAG, ncores, beta_obs)
      ...
      V_next <-  fit_V_VU_const(V, U_next, theta, rho, lambda, solve_its, DAG, ncores, beta_obs)
...
fit_inspre_sequence <- function(X, lambda, W = NULL, rho = 1.0,
                                ...
                                warm_start = FALSE, constraint = "UV", DAG = FALSE, beta_obs = NULL) {
  ...
  for (i in 1:length(lambda)) {
    ...
    inspre_res <- inspre_worker(
      ...
      solve_its = solve_its, ncores = ncores, constraint = constraint, DAG = DAG, beta_obs = beta_obs
    )
    if (verbose) {
      cat(sprintf(
        paste("Converged to L = %f in %d iterations for lambda = %f.",
              " Time %f.",
  ...
  return(list(
    "V" = V_all, "U" = U_all, "lambda" = lambda, "gamma" = gamma, "rho" = rho_used,
...
inspre <- function(X, W = NULL, rho = 10.0, lambda = NULL,
                   ...
                   warm_start = TRUE, min_nz = 0.01, constraint = "UV", DAG = FALSE, beta_obs = NULL) {
  D <- ncol(X)
  print("Local inspre function is called")
  if (any(is.na(X))) {
    if (is.null(W)) {
      W <- matrix(1L, nrow = D, ncol = D)
    }
  ...
  if (!is.null(alpha)) {
    ...
    if (alpha > 0) {
      ...
      L0 <- fit_inspre_sequence(
        ...
        ncores = ncores, warm_start = warm_start, constraint = constraint, DAG = DAG, beta_obs = beta_obs
      )$L
  ...
  if (cv_folds > 0) {
    ...
    for (i in 1:cv_folds) {
      ...
      cv_res <- fit_inspre_sequence(
        ...
        solve_its = solve_its, ncores = ncores, warm_start = warm_start, constraint = constraint, DAG = DAG, beta_obs = beta_obs
      )
...
fit_inspre_from_R <- function(R_tce, W = NULL, rho = 100.0, lambda = NULL,
                              ...
                              ncores = 1, warm_start = FALSE, min_nz = 0.01, constraint = "UV", DAG = FALSE, beta_obs = NULL) {
  D <- dim(R_tce)[1]
  inspre_res <- # inspre::
    inspre(
      ...
      tau = tau, solve_its = solve_its, ncores = ncores, warm_start = warm_start, min_nz = min_nz, constraint = constraint, DAG = DAG, beta_obs = beta_obs
    )
...
fit_inspre_from_X <- function(X, targets, weighted = TRUE, max_med_ratio = NULL, filter = TRUE,
  ...
  if (weighted) {
    W <- make_weights(SE_hat, max_med_ratio = max_med_ratio)
  } else {
    W <- NULL
  }
  
  full_res <- fit_inspre_from_R(R_hat,
                                W = W, rho = rho, lambda = lambda,
                                lambda_min_ratio = lambda_min_ratio, nlambda = nlambda, alpha = alpha,
                                gamma = gamma, its = its, delta_target = delta_target,
                                verbose = verbose, train_prop = 1,
                                cv_folds = 0, mu = mu, tau = tau, solve_its = solve_its,
                                ncores = ncores, warm_start = warm_start, min_nz = min_nz, constraint = constraint, DAG = DAG, beta_obs = beta_obs
  )
  ...
  if (cv_folds > 0) {
    ...
    for (i in seq_along(folds)) {
      ...
      if (weighted) {
        W_cv <- make_weights(SE_hat_cv, max_med_ratio = max_med_ratio)
      } else {
        W_cv <- NULL
      }
      cv_res <- fit_inspre_from_R(R_hat_cv,
                                  W = W_cv, rho = rho, lambda = lambda,
                                  lambda_min_ratio = lambda_min_ratio, nlambda = nlambda, alpha = alpha,
                                  gamma = gamma, its = its, delta_target = delta_target,
                                  verbose = verbose, train_prop = 1,
                                  cv_folds = 0, mu = mu, tau = tau, solve_its = solve_its,
